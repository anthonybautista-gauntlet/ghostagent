...
async function run() {
  ...
  const datasetPath = resolvePathFromLib({
    overridePath: process.env.AGENTFORGE_REPLAY_DATASET_PATH,
    pathFromLib: 'staged/golden-cases.json'
  });
  ...
  const outputDirectory = resolve(resolveEvalHistoryDirectoryPath(), 'replay');
